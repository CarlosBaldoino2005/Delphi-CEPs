...
type
  TUriCEPAttribute = class(TCustomAttribute)
  private
    FName: string;
    FUri: string;
  public
...
begin
  ...
  begin
    if lAttribute is TUriCEPAttribute then
    begin
      FName := TUriCEPAttribute(lAttribute).Name;
      FUri  := TUriCEPAttribute(lAttribute).Uri;
    end;
  end;
end;
...
begin
  Result := StringReplace(FUri,'<#Code>',FCode,[]);
end;
...
constructor TUriCEPAttribute.Create(aName, aUri: string);
begin
  FName := aName;
  FUri  := aUri;
end;
